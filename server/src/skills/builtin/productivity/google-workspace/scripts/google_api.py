#!/usr/bin/env python3
"""Google Workspace API CLI for dasha Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing dasha-facing JSON contract and falls back to the Python client
...
from _dasha_home import get_dasha_home
...
def _gws_binary() -> str | None:
    override = os.getenv("dasha_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for dasha Agent")
