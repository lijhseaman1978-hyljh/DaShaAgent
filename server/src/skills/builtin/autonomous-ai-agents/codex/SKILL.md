...
metadata:
  dasha:
    ...
    related_skills: [claude-code, dasha-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the dasha terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For dasha itself, `model.provider: openai-codex` uses dasha-managed Codex
OAuth from `~/.dasha/auth.json` after `dasha auth add openai-codex`. For the
...
## dasha Gateway Caveat

When invoking the Codex CLI from a dasha gateway/service context (for example,
