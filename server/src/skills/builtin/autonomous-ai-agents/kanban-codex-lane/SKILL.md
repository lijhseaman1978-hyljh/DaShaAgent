...
description: Use when a dasha Kanban worker wants to run Codex CLI as an isolated implementation lane while dasha keeps ownership of task lifecycle, reconciliation, testing, and handoff.
version: 1.0.0
author: dasha Agent
license: MIT
metadata:
  dasha:
    ...
    related_skills: [kanban-worker, codex, dasha-agent]
---
...
This skill defines the lightweight dasha+Codex dual-lane convention for Kanban workers. dasha is always the task owner: it calls `kanban_show`, decides whether Codex is appropriate, creates or selects an isolated workspace, starts and monitors Codex, reconciles any diff, runs verification, and writes the final `kanban_complete` or `kanban_block` handoff. Codex is an input lane only. Codex output is not a task completion signal, not a trusted reviewer, and not allowed to write durable Kanban state directly.

The convention exists so a dasha worker can use Codex for bounded implementation help without changing the dispatcher. The dispatcher must still spawn dasha workers. A worker may optionally spawn Codex inside its own run, then accept, partially accept, or reject the lane after independent review and tests.
...
- A bounded diff can be evaluated by dasha in one run.
- The repo can be copied or checked out in an isolated git worktree/branch.
- dasha can run the relevant tests itself after Codex exits.
...
1. dasha owns the Kanban lifecycle. Codex must never call `kanban_complete`, `kanban_block`, `kanban_create`, gateway messaging, or any dasha board CLI as a substitute for the worker.
2. dasha owns final acceptance. Treat Codex commits/diffs as untrusted patches until reviewed and verified.
3. dasha owns test execution. Codex may run tests, but those runs are advisory; repeat required verification from dasha with the repo's canonical wrapper.
4. dasha owns safety. If Codex changes safety boundaries, risk gates, live trading behavior, or secrets handling, reject the lane even if tests pass.
5. dasha owns cleanup. Kill stuck Codex processes and remove temporary worktrees when they are no longer needed.
...
```bash
TASK_ID="${dasha_KANBAN_TASK:-t_manual}"
REPO="/path/to/repo"
...
If the current Kanban workspace is already an isolated git worktree created for this task, you may create a sibling Codex branch inside it only if `git status --short` is clean except for intentional dasha edits. Otherwise create a separate temporary worktree and cherry-pick or copy accepted commits back after reconciliation.
...
Run these before spawning Codex. Missing Codex is a normal reason to skip the lane, not a task blocker if dasha can do the task directly.
...
dasha owns the Kanban lifecycle; do not call dasha kanban tools or messaging.
...
Do not use `--yolo` for prediction-market-bot or safety-sensitive repos. Prefer `--full-auto` inside the isolated worktree, then rely on dasha reconciliation.
...
- Explicit statement: dasha owns Kanban lifecycle; Codex is an input lane only.
...
- Verification commands Codex may run and commands dasha will run afterward.
...
dasha must perform this checklist before accepting any Codex lane result:

- [ ] `git -C <WORKTREE> status --short --branch` shows only expected files.
- [ ] `git -C <WORKTREE> diff --stat` and `git diff` were reviewed by dasha.
...
- [ ] dasha ran the canonical tests itself, using `scripts/run_tests.sh` for dasha Agent or the repo's documented wrapper for other repos.
- [ ] Any Codex-run tests are listed separately from dasha-run tests.
- [ ] Accepted commits/diffs were applied to the dasha-owned workspace/branch.
...
```json
{
  "codex_lane": {
    ...
    "tests_run": [
      {"command": "scripts/run_tests.sh tests/tools/test_x.py", "exit_code": 0, "owner": "dasha"},
...
```json
{
  "codex_lane": {
    ...
    "result": "rejected",
    "accepted_commits": [],
    "rejected_reason": "Direct dasha edit was smaller and safer than spawning Codex.",
    "tests_run": [],
    "artifacts": []
  }
}
...
1. Treating Codex self-report as verification. Always inspect the diff and rerun tests from dasha.
2. Running Codex in the user's dirty main checkout. Always isolate in a worktree/branch.
3. Letting Codex own Kanban. Codex may summarize progress, but dasha writes board state.
...
- [ ] dasha reviewed `git diff` and safety-sensitive files.
- [ ] dasha ran canonical tests independently.
