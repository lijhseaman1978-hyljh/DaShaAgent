---
name: debugging-dasha-tui-commands
description: "Debug dasha TUI slash commands: Python, gateway, Ink UI."
version: 1.0.0
author: dasha Agent
license: MIT
...
metadata:
  dasha:
    tags: [debugging, dasha-agent, tui, slash-commands, typescript, python]
    related_skills: [python-debugpy, node-inspect-debugger, systematic-debugging]
---

# Debugging dasha TUI Slash Commands

## Overview

dasha slash commands span three layers — Python command registry, tui_gateway JSON-RPC bridge, and the Ink/TypeScript frontend. When a command misbehaves (missing from autocomplete, works in CLI but not TUI, config persists but UI doesn't update), the bug is almost always one layer being out of sync with another.

Use this skill when you encounter issues with slash commands in the dasha TUI, particularly when commands aren't showing in autocomplete, aren't working properly in the TUI, or need to be added/updated.
...
```
Python backend (dasha_cli/commands.py)     <- canonical COMMAND_REGISTRY
       │
       ▼
...
3. **Check if the command exists in the Python backend:**
   ```bash
   search_files --pattern "CommandDef" --file_glob "*.py" --path dasha_cli/
   search_files --pattern "commandname" --path dasha_cli/commands.py --context 3
   ```
...
1. Add a `CommandDef` entry to `COMMAND_REGISTRY` in `dasha_cli/commands.py`:
   ```python
...
4. If the command runs server-side, add a handler in `dashaCLI.process_command()` in `cli.py`:
   ```python
...
1. **Command shows in TUI but not in autocomplete.** The command is defined in the TUI codebase but missing from `COMMAND_REGISTRY` in `dasha_cli/commands.py`. Autocomplete data ships from Python.
...
1. Rebuild the TUI:
   ```bash
   cd /home/bb/dasha-agent && npm --prefix ui-tui run build
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.dasha/config.yaml`)
