...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _dasha_env="${dasha_HOME:-$HOME/.dasha}/.env"; [ -f "$_dasha_env" ] && grep -q "^GITHUB_TOKEN=" "$_dasha_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_dasha_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from dasha Agent\",
    \"comments\": [
...
```bash
source "${dasha_HOME:-$HOME/.dasha}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by dasha Agent. Code looks clean — good test coverage, no security concerns."
