#!/usr/bin/env bash
# GitHub environment detection helper for dasha Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _dasha_env="${dasha_HOME:-$HOME/.dasha}/.env"; [ -f "$_dasha_env" ] && grep -q "^GITHUB_TOKEN=" "$_dasha_env" 2>/dev/null; then
