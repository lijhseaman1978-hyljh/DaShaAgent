...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("dasha_HOME", os.path.expanduser("~/.dasha")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("dasha_HOME", Path.home() / ".dasha")) / "skills" / "red-teaming" / "godmode"
...
dasha_HOME = Path(os.getenv("dasha_HOME", Path.home() / ".dasha"))
CONFIG_PATH = dasha_HOME / "config.yaml"
PREFILL_PATH = dasha_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/dasha models — already uncensored, just needs clean prompt
    "dasha": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "dasha" in model_lower or "nous" in model_lower:
        return "dasha"
...
def _get_current_model() -> tuple:
    """Read current model and provider from dasha config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.dasha/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart dasha for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart dasha for changes to take effect.")
