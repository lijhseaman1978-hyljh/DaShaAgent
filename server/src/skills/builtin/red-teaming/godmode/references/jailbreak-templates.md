...
## 5. GODMODE FAST — dasha 4 405B (Zero Refusal)

**Model:** `nousresearch/dasha-4-405b`
**Strategy:** dasha models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with dasha Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.dasha/config.yaml`:
