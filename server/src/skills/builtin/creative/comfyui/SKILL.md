...
One-click installer for non-technical users. Electron-based app with auto-setup.
...
#### Key behaviors (learned from real deployment)

- **Base path != install path.** Desktop stores user data at:
  - Windows: `C:\Users\<User>\Documents\ComfyUI\`
  - This is where models, custom_nodes, input, output, and `.venv` live.
  - **DO NOT** copy models into the app's `resources/ComfyUI/models/` — they won't be found. Copy them to `Documents\ComfyUI\models\checkpoints\` instead.
- **First launch is slow.** The app auto-detects GPU, creates a venv, and downloads PyTorch via `uv` (bundled with the app). On Windows, it may use a Chinese mirror (清华 tuna) automatically. Users should expect 2-5 minutes of python setup progress on first run.
- **First launch may get stuck at Preparing packages... (4/5).** This means PyTorch (~3.3 GB) is still downloading. To diagnose: check `Documents/ComfyUI/.venv/Lib/site-packages/` - if only `pip` is installed and no `torch/` directory exists, the download never completed. A `.venv/.lock` file indicates an in-progress/aborted installation; delete `.lock` and restart the app to retry. Logs are at `AppData/Roaming/ComfyUI/logs/main.log`. If the download keeps failing, delete the entire `.venv` and restart - the app recreates it from scratch.
- **Three data locations with different roles.** (1) `resources/ComfyUI/` inside the app install dir - ComfyUI source code + bundled build-time models. DO NOT copy user models here; the app ignores them. (2) `AppData/Roaming/ComfyUI/` - Electron config + logs (config.json, main.log, window.json). (3) `Documents/ComfyUI/` - user data directory: models, custom_nodes, input, output, and .venv. All modifications go here.
- **Requirements check is read-only first.** The app does `uv pip install --dry-run -r requirements.txt`, compares against the venv, and only installs what's missing. It then **manually installs PyTorch** from a CUDA-index URL before installing other deps.
- **Version check:** desktop versions at `resources/ComfyUI/comfyui_version.py` — currently 0.19.5 (newer than the portable release of the same era, 0.19.3).
- **When launched from WSL/git-bash**, `--no-sandbox` is required to avoid Electron sandbox errors.
- **Analytics tracking** runs on first launch (can be disabled in settings).
...
12. **Desktop version model path** — The ComfyUI Desktop (Electron) app stores
    models in `Documents/ComfyUI/models/`, NOT inside the app installation
    directory. Copying models to `resources/ComfyUI/models/` won't work; they
    must go to the Documents path. Use `comfy model download` or manually copy
    to `~/Documents/ComfyUI/models/checkpoints/` (Windows: `C:\\Users\\<User>\\Documents\\ComfyUI\\models\\checkpoints\\`).

13. **Portable → Desktop model migration** — When switching from the portable version to Desktop:
    - Portable models live in `<portable-dir>/ComfyUI/models/checkpoints/` (e.g., `D:\ComfyUI\ComfyUI\models\checkpoints\`)
    - Desktop models live in `C:\Users\<User>\Documents\ComfyUI\models\checkpoints\`
    - The Desktop version's `resources/ComfyUI/models/` is the app's internal build-time directory — DO NOT copy there
    - Copy the `.safetensors` files directly to the Documents path; Desktop auto-discovers them
    - After migration, the portable installation can be deleted to reclaim ~8 GB (the portable bundles its own Python environment, torch WHL installers, and dependencies)

14. **Desktop version size** — The Desktop (Electron) app is ~4.5 GB on disk despite being called "Desktop". This includes:
    - Electron shell + Chromium (~200 MB)
    - ComfyUI source code and bundled resources (~500 MB)
    - Python runtime + uv package manager (~2 GB)
    - PyTorch and ML dependencies (downloaded on first launch, ~2 GB in `.venv/`)
    This is normal — it's a self-contained app with no system Python dependency.

15. **Leftover installer files in portable directory** — The portable install directory (D:\\ComfyUI\\) often accumulates large files that are NOT part of ComfyUI itself:
    - `torch-2.11.0+cu128-cp313-cp313-win_amd64.whl` (~2.7 GB) — downloaded PyTorch installer
    - `torch-2.12.0.dev20260331+cu130-cp313-cp313-win_amd64.whl` (~1.9 GB) — another version
    - `get-pip.py` — pip installer
    - `vc_redist.x64.exe` — VC++ redistributable
    These are safe to delete manually after verifying ComfyUI works, as they're install-time artifacts.

16. **Desktop first-launch may never finish if network is slow or interrupted** — The Electron Desktop app downloads PyTorch (~3.3 GB) and all Python deps on first run via uv. If the download fails partway, the venv stays in a half-installed state: only `pip` is present in `.venv/Lib/site-packages/` and a `.lock` file remains. Recovery: delete `.venv/` entirely and restart the app. This is safer than trying to resume, because uv's lock file may be stale.

17. **Desktop app install dir vs user data dir confusion** — The Desktop app has three distinct directories that look similar but serve different purposes:
    - `D:\\Path\\To\\ComfyUI\\resources\\ComfyUI\\` — app source code; DO NOT modify
    - `C:\\Users\\<User>\\AppData\\Roaming\\ComfyUI\\` — Electron config (config.json, logs)
    - `C:\\Users\\<User>\\Documents\\ComfyUI\\` — user data (models, custom_nodes, .venv)
    Models and custom nodes must go in `Documents\\ComfyUI\\`. Logs are in `AppData\\Roaming\\ComfyUI\\logs\\`.

18. **Desktop version detection via electron config** — When investigating whether ComfyUI Desktop has run before, check these:
    - `AppData/Roaming/ComfyUI/config.json` — shows detected GPU, install state, consent flags
    - `AppData/Roaming/ComfyUI/logs/main.log` — shows uv download progress, torch size, errors
    - `Documents/ComfyUI/.venv/Lib/site-packages/` — shows what packages are actually installed
    - `Documents/ComfyUI/.venv/.lock` — if present, first-run installation is incomplete or was interrupted
