...
dasha_HOME_DIR="${dasha_HOME:-$HOME/.dasha}"
dasha_CFG="${dasha_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure dasha config has twozero_td MCP entry ──
if [[ ! -f "$dasha_CFG" ]]; then
    echo -e " ${FAIL} dasha config not found at ${dasha_CFG}"
    manual_steps+=("Create ${dasha_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$dasha_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in dasha config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to dasha config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${dasha_CFG} manually"); }
    manual_steps+=("Restart dasha session to pick up config change")
fi
