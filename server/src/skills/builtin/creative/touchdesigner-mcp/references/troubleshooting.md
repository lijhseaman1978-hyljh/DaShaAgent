...
## 5. dasha Configuration

### Config location

`$dasha_HOME/config.yaml` (defaults to `~/.dasha/config.yaml` when `dasha_HOME` is unset)
...
Restart the dasha session for changes to take effect. The MCP connection is
