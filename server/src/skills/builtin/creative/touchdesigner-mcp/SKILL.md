...
metadata:
  dasha:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, dasha-video]
...
```
dasha Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${dasha_HOME:-$HOME/.dasha}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to dasha config (if missing)
...
3. **Restart dasha session** to pick up the new MCP server
