...
This skill is separate from the dasha Email gateway adapter. The gateway
adapter lets people email the agent and uses dasha' built-in IMAP/SMTP
...
To reply non-interactively from dasha, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from dasha)** — pipe the message via stdin:
