...
function fmtTime(ts) { if (!ts) return ''; var d = new Date(ts), diff = Date.now() - d.getTime(); var min = 60000, hour = 3600000, day = 86400000; if (diff < 0 || diff > 365 * day) return d.toLocaleString(getLang() === 'en' ? 'en-US' : 'zh-CN', { hour12: false }); if (diff < min) return t('time_now'); if (diff < hour) return t('time_min', { n: Math.floor(diff / min) }); if (diff < day) return t('time_hour', { n: Math.floor(diff / hour) }); if (diff < 7 * day) return t('time_day', { n: Math.floor(diff / day) }); return d.toLocaleString(getLang() === 'en' ? 'en-US' : 'zh-CN', { hour12: false }); }
function updateSessionTitle(t) { var el = $('sessionTitle'); if (el) el.textContent = t || window.t('new_chat'); }
function updateMsgCount() { var el = $('msgCount'); if (!el) return; el.textContent = t('msg_count', { n: chat.querySelectorAll('.msg').length }); }
function fmtUptime(s) { s = Math.floor(s); var h = Math.floor(s / 3600), m = Math.floor((s % 3600) / 60), sec = s % 60; return (h ? h + t('up_h') : '') + m + t('up_m') + sec + t('up_s'); }
function showToast(msg, type) { type = type || ''; var c = $('toastContainer'); if (!c) return; var t = document.createElement('div'); t.className = 'toast toast-' + type; t.innerHTML = '<span>' + escapeHtml(msg) + '</span><button class="toast-close" aria-label="' + t('close') + '">X</button>'; t.querySelector('.toast-close').onclick = function() { t.remove(); }; c.appendChild(t); setTimeout(function() { if (t.parentNode) t.remove(); }, 3000); }
...
function updateCtxBadge(extraDraft) { extraDraft = extraDraft || 0; var el = $('ctxTokens'); if (!el) return; var total = currentTokens + extraDraft; var pct = Math.min(100, Math.round(total / CTX_DANGER * 100)); el.classList.toggle('warn', total >= CTX_WARN && total < CTX_DANGER); el.classList.toggle('danger', total >= CTX_DANGER); el.innerHTML = '<span class="ctx-bar" style="width:' + pct + '%"></span><span class="ctx-txt">' + t('ctx_tokens_val', { n: total.toLocaleString() }) + '</span>'; }
...
function addMsg(role, text) { var el = document.createElement('div'); el.className = 'msg ' + role; var label = role === 'user' ? (chatUI.labelUser || t('label_you')) : role === 'assistant' ? (chatUI.labelAgent || t('label_assist')) : t('system_role'); el.setAttribute('aria-label', label + '消息'); if (role !== 'system') { var avEl = document.createElement('div'); avEl.className = 'msg-avatar'; renderAvatarInto(avEl, role); avEl.setAttribute('aria-hidden', 'true'); el.appendChild(avEl); } var main = document.createElement('div'); main.className = 'msg-main'; main.innerHTML = '<div class="who">' + label + '</div><div class="bubble"></div>'; if (role === 'assistant' && !text) { main.querySelector('.bubble').innerHTML = '<div class="typing-indicator"><span></span><span></span><span></span></div>'; } else { main.querySelector('.bubble').innerHTML = renderMarkdownFull(text); enhanceCodeBlocks(main.querySelector('.bubble')); } el.appendChild(main); chat.appendChild(el); addMsgActions(el, role, text); updateMsgCount(); scrollDown(true); return el; }
...
function setBusy(b) { busy = b; sendBtn.classList.toggle('stop', b); var si = sendBtn.querySelector('.send-icon'); if (si) { if (b) { si.textContent = '\u25a0'; } else { si.innerHTML = SEND_SVG; } } sendBtn.title = b ? t('stop') : t('send'); }
...
function selectModel(id) { currentModelId = id; updateModelTrigger(); $('modelDropdown').hidden = true; var lbl = labelOf(id); if (lbl) setConn(t('connected') + ' · ' + lbl, 'ok'); var fm = $('footModel'); if (fm) fm.textContent = lbl || '—'; fetch('/api/config', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ activeModelId: id }) }).catch(function() {}); }
function updateModelTrigger() { $('modelName').textContent = labelOf(currentModelId) || t('model_trigger'); }
...
var _dashBtn = $('dashBtn'); if (_dashBtn) _dashBtn.onclick = function() { if (currentView === 'dashboard') switchView('chat'); else switchView('dashboard'); };
var _refreshTop = $('refreshTopBtn'); if (_refreshTop) _refreshTop.onclick = function() { loadModels(); refreshTokens(); showToast(t('refreshed'), 'success'); };
$('teamBtn').onclick = function() { var task = prompt(t('team_task_prompt') || '多智能体任务:'); if (!task) return; addMsg('system', t('team_task_start') + task); ws.send(JSON.stringify({ type: 'team', task: task, sessionId: sessionId })); };

/* ─── 压缩 (HTTP fallback) ─── */
$('compressBtn').onclick = async function() {
  if (busy) return;
  if (!confirm(t('compress_confirm'))) return;
  var btn = $('compressBtn'), orig = btn.textContent;
  btn.disabled = true; btn.textContent = t('compressing'); showToast(t('compressing_ctx'));
  try {
    ...
    if (r.ok) { showToast(r.fallback ? t('compress_fallback') : t('compress_done')); loadSession(sessionId); }
    else showToast(t('compress_fail') + (r.error || ''));
  } catch { showToast(t('compress_fail_net')); }
...
async function renderSettings() {
  ...
  body.innerHTML =
    '    <div class="card"><div class="card-h">' + t('s_theme') + '</div>' +
    '      <div class="field"><div class="seg" id="themeSeg">' +
    '        <button data-v="system">' + t('theme_system') + '</button>' +
    '        <button data-v="light">' + t('theme_light') + '</button>' +
    '        <button data-v="dark">' + t('theme_dark') + '</button>' +
    '      </div></div>' +
    '    </div>' +
    '    <div class="card"><div class="card-h">' + t('s_font') + '</div>' +
    '      <div class="field"><div class="seg" id="fontSeg">' +
    '        <button data-v="13">' + t('font_small') + '</button>' +
    '        <button data-v="14">' + t('font_std') + '</button>' +
    '        <button data-v="15">' + t('font_large') + '</button>' +
    '        <button data-v="16">' + t('font_xl') + '</button>' +
    '      </div></div>' +
    '    </div>' +
    '    <div class="card"><div class="card-h">' + t('s_voice') + '</div>' +
    '      <div class="set-switches">' +
    '        <label class="switch"><input type="checkbox" id="setVoiceInput" /><span>' + t('voice_input') + '</span></label>' +
    '        <label class="switch"><input type="checkbox" id="setVoiceTTS" /><span>' + t('voice_tts') + '</span></label>' +
    '      </div>' +
    '    </div>' +
    '    <div class="card"><div class="card-h">' + t('s_roles') + '</div>' +
    '      <div class="field"><input id="setLabelUser" placeholder="' + t('my_name_ph') + '" /></div>' +
    '      <div class="field"><input id="setLabelAgent" placeholder="' + t('agent_name_ph') + '" /></div>' +
    '    </div>' +
    '    <div class="card"><div class="card-h">' + t('s_data') + '</div>' +
    '      <div class="field set-actions">' +
    '        <button class="mini-btn" id="expChatBtn">' + t('export_chat') + '</button>' +
    '        <button class="mini-btn" id="clearChatBtn">' + t('clear_chat2') + '</button>' +
    '        <button class="mini-btn danger" id="resetUiBtn">' + t('reset_ui') + '</button>' +
    '      </div>' +
    '    </div>' +
    '    <div class="card about-card"><div class="card-h">' + t('s_about') + '</div>' +
    '      <div class="about-row">DaShaAgent <span class="muted">' + escapeHtml(ver) + '</span></div>' +
    '      <div class="about-row muted">' + t('about_sub') + '</div>' +
    '    </div>';
  ...
  // 对话角色称呼
  $('setLabelUser').value = chatUI.labelUser || t('label_you');
  $('setLabelAgent').value = chatUI.labelAgent || t('label_assist');
  $('setLabelUser').onchange = function(e) { chatUI.labelUser = e.target.value.trim() || t('label_you'); saveUI(); refreshRoleLabels(); };
  $('setLabelAgent').onchange = function(e) { chatUI.labelAgent = e.target.value.trim() || t('label_assist'); saveUI(); refreshRoleLabels(); };

  // 数据管理
  $('expChatBtn').onclick = function() { if ($('exportBtn')) $('exportBtn').click(); };
  $('clearChatBtn').onclick = function() {
    if (confirm(t('clear_confirm'))) { chat.innerHTML = ''; updateMsgCount(); scrollDown(); showToast(t('deleted'), 'success'); }
  };
...
function refreshRoleLabels() {
  ...
  chat.querySelectorAll('.msg').forEach(function(m) {
    ...
    w.textContent = m.classList.contains('user') ? (chatUI.labelUser || t('my_name') || '你') : m.classList.contains('assistant') ? (chatUI.labelAgent || t('agent_name') || '助手') : t('system_role');
  });
}
...
function renderDefaultModelList() {
  ...
  fetch('/api/models').then(function(r) { return r.json(); }).then(function(md) {
    ...
    (md.groups || []).forEach(function(g) {
      (g.providers || []).forEach(function(p) {
        p.models.forEach(function(m) {
          ...
          row.innerHTML = '<div class="lr-main"><div class="lr-title">' + escapeHtml(m.label) + '</div><div class="lr-sub">' + escapeHtml(g.label) + ' · ' + escapeHtml(p.label) + '</div></div>' + (sel ? '<span class="badge">' + t('default_badge') + '</span>' : '');
    ...
    if (!found && _activeModelId) {
      ...
      row.innerHTML = '<div class="lr-main"><div class="lr-title">' + escapeHtml(_activeModelId) + '</div><div class="lr-sub">' + t('active_unlisted') + '</div></div><span class="badge">' + t('default_badge') + '</span>';
      wrap.appendChild(row);
    }
  });
}
...
var NAV = ['chat','history','memory','tasks','models','skills','marketplace','plugins','config','logs','metrics','health','settings'];
var PANELS = {
  history: { title: t('p_history'), render: renderHistory },
  memory: { title: t('p_memory'), render: renderMemory },
  tasks: { title: t('p_tasks'), render: renderTasks },
  models: { title: t('p_models'), render: renderModels },
  skills: { title: t('p_skills'), render: renderSkills },
  plugins: { title: t('p_plugins'), render: renderPlugins },
  config: { title: t('p_config'), render: renderConfig },
  logs: { title: t('p_logs'), render: renderLogs },
  metrics: { title: t('p_metrics'), render: renderMetrics },
  health: { title: t('p_health'), render: renderHealth },
  settings: { title: t('p_settings'), render: renderSettings },
  dashboard: { title: t('p_dashboard'), render: renderDashboard },
};
...
function buildNav() { var nav = $('nav'); if (!nav) return; nav.innerHTML = ''; var active = currentView || 'chat'; for (var i = 0; i < NAV.length; i++) { var v = NAV[i]; var ic = NAV_ICONS[v] || { c: '#9aa5b5', svg: '' }; var b = document.createElement('button'); b.className = 'nav-item' + (v === active ? ' active' : ''); b.dataset.view = v; b.style.setProperty('--ico', ic.c); b.style.setProperty('--ico-soft', hexToRgba(ic.c, 0.16)); b.innerHTML = '<span class="nav-ico">' + ic.svg + '</span><span class="nav-label">' + t('nav_' + v) + '</span>'; b.onclick = function() { switchView(this.dataset.view); }; nav.appendChild(b); } }
function switchView(view) {
  ...
  else { $('viewChat').hidden = true; $('viewPanel').hidden = false; $('panelTitle').textContent = t('p_' + view); $('panelActions').innerHTML = ''; PANELS[view].render(); }
}
...
async function renderHistory() {
  ...
  var body = $('panelBody');
  if (!list.length) { body.innerHTML = '<div class="empty-panel">' + t('no_history') + '</div>'; return; }
  body.innerHTML = '';
  for (var i = 0; i < list.length; i++) {
    var s = list[i];
    var item = document.createElement('div'); item.className = 'list-row';
    item.innerHTML = '<div class="lr-main"><div class="lr-title">' + escapeHtml(s.title || t('new_chat')) + '</div><div class="lr-sub">' + fmtTime(s.updatedAt) + '</div></div>' +
      '<div class="row" style="margin:0"><button class="mini-btn open-session">' + t('open') + '</button><button class="mini-btn ghost rename-session">' + t('rename') + '</button></div>';
    (function(sid, sitem, stitle) {
      sitem.querySelector('.open-session').onclick = function() { switchView('chat'); loadSession(sid); };
      sitem.querySelector('.rename-session').onclick = function() { startRename(sitem, sid, stitle || t('new_chat')); };
    })(s.id, item, s.title);
...
function startRename(item, id, title) {
  ...
  var commit = async function() {
    ...
    if (r.ok) renderHistory(); else { showToast(t('rename_fail'), 'error'); renderHistory(); }
  };
...
async function renderMemory() {
  ...
  var html = '';
  html += '<div class="card"><div class="card-h">' + t('mem_profile') + '</div>';
  html += '<textarea class="profile-editor" id="profileEditor" spellcheck="false">' + escapeHtml(JSON.stringify(profile, null, 2)) + '</textarea>';
  html += '<div class="editor-actions"><button class="mini-btn" id="saveProfile">' + t('save') + '</button><span class="muted" id="profileMsg"></span></div></div>';
  html += '<div class="card"><div class="card-h">' + t('mem_notes', { n: notes.length }) + '</div>';
  for (var i = 0; i < notes.length; i++) {
    var n = notes[i];
    html += '<div class="note-item" data-topic="' + escapeHtml(n.topic) + '">';
    html += '<div class="note-item-h"><span class="note-item-title">' + escapeHtml(n.topic) + '</span><button class="mini-btn danger note-del" data-topic="' + escapeHtml(n.topic) + '">' + t('del') + '</button></div>';
    html += '<textarea class="note-editor" data-topic="' + escapeHtml(n.topic) + '" spellcheck="false">' + escapeHtml(n.content || '') + '</textarea>';
    html += '<div class="editor-actions" style="margin-bottom:0"><button class="mini-btn note-save" data-topic="' + escapeHtml(n.topic) + '">' + t('save') + '</button></div>';
    html += '</div>';
  }
  html += '</div>';
  html += '<div class="card"><div class="card-h">' + t('mem_new') + '</div><div class="field"><label>' + t('mem_topic_ph') + '</label><input id="newTopic" placeholder="' + t('mem_topic_ph') + '"/></div><textarea id="newContent" placeholder="' + t('mem_content_ph') + '"></textarea><div class="editor-actions" style="margin-top:8px;margin-bottom:0"><button class="mini-btn" id="createNote">' + t('mem_create') + '</button></div></div>';
  html += '<div class="card"><div class="card-h">' + t('mem_recall') + '</div><div class="row"><input id="recallQ" placeholder="' + t('mem_recall_ph') + '"/><button class="mini-btn" id="recallBtn">' + t('mem_recall_btn') + '</button></div><div class="recall-out" id="recallOut"></div></div>';
  body.innerHTML = html;

  $('saveProfile').onclick = async function() {
    var raw = $('profileEditor').value; var obj;
    try { obj = JSON.parse(raw); } catch (e) {
      var m = $('profileMsg'); m.textContent = t('json_err') + ': ' + e.message; m.style.color = 'var(--danger)'; return;
    }
    ...
    var m = $('profileMsg');
    if (r.ok) { m.textContent = t('saved'); m.style.color = 'var(--ok)'; }
    else { m.textContent = t('save_fail') + ': ' + (r.error || ''); m.style.color = 'var(--danger)'; }
  };
  body.querySelectorAll('.note-save').forEach(function(b) { b.onclick = async function() {
    ...
    showToast(r.ok ? t('note_saved') : t('save_fail'));
  }; });
  body.querySelectorAll('.note-del').forEach(function(b) { b.onclick = async function() {
    ...
    if (r.ok) showToast(t('deleted'), 'success'); else showToast(t('save_fail'), 'error');
    renderMemory();
  }; });
  $('createNote').onclick = async function() {
    var topic = $('newTopic').value.trim(); var content = $('newContent').value;
    if (!topic) { showToast(t('fill_topic'), 'warning'); return; }
    ...
    if (r.ok) { $('newTopic').value = ''; $('newContent').value = ''; showToast(t('note_created'), 'success'); renderMemory(); }
    else showToast(t('create_fail'), 'error');
  };
  $('recallBtn').onclick = async function() {
    ...
    $('recallOut').innerHTML = (r.items || []).map(function(t) { return '<div class="recall-item">' + escapeHtml(t) + '</div>'; }).join('') || t('no_result');
  };
}

/* ─── 面板: 任务 ─── */
var editingJobName = null;
var WEEK_OPTS = [['MON', t('mon')], ['TUE', t('tue')], ['WED', t('wed')], ['THU', t('thu')], ['FRI', t('fri')], ['SAT', t('sat')], ['SUN', t('sun')]];
...
function jobFormHtml(prefix, job) { var c = job ? parseCronStr(job.cron) : { freq: 'daily', time: '07:00', week: 'MON', day: '1' }; return '<div class="job-form"><div class="row"><input id="' + prefix + 'Name" placeholder="' + t('job_name_ph') + '" value="' + (job ? escapeHtml(job.name) : '') + '"/></div><div class="row"><select id="' + prefix + 'Freq"><option value="daily"' + (c.freq === 'daily' ? ' selected' : '') + '>' + t('freq_daily') + '</option><option value="weekly"' + (c.freq === 'weekly' ? ' selected' : '') + '>' + t('freq_weekly') + '</option><option value="monthly"' + (c.freq === 'monthly' ? ' selected' : '') + '>' + t('freq_monthly') + '</option></select><input type="time" id="' + prefix + 'Time" value="' + escapeHtml(c.time) + '" style="flex:0 0 120px"/><span id="' + prefix + 'WeekWrap" style="display:' + (c.freq === 'weekly' ? 'inline-flex' : 'none') + ';flex:0 0 auto">' + weekSelect(prefix + 'Week', c.week) + '</span><input type="number" id="' + prefix + 'Day" min="1" max="31" value="' + escapeHtml(c.day) + '" placeholder="' + t('day_ph') + '" style="display:' + (c.freq === 'monthly' ? 'block' : 'none') + ';flex:0 0 90px"/></div><textarea id="' + prefix + 'Prompt" placeholder="' + t('job_prompt_ph') + '">' + (job ? escapeHtml(job.prompt) : '') + '</textarea></div>'; }
...
async function renderTasks() {
  ...
  var jobs = d.jobs || [];
  var body = $('panelBody');
  var html = '<div class="card"><div class="card-h">' + t('tasks_title', { n: jobs.length }) + '<button class="mini-btn" id="addJobBtn" style="float:right">' + t('tasks_new') + '</button></div>';
  html += '<div id="jobFormWrap"></div>';
  for (var i = 0; i < jobs.length; i++) {
    var j = jobs[i];
    var enabled = j.enabled !== false;
    var statusBadge = enabled ? '<span class="tag tag-ok" style="margin-right:4px">' + t('running') + '</span>' : '<span class="tag tag-bad" style="margin-right:4px">' + t('paused') + '</span>';
    ...
    html += '<button class="mini-btn run-job" data-name="' + escapeHtml(j.name) + '" title="' + t('run_now') + '">' + t('run_now') + '</button>';
    html += '<button class="mini-btn ' + (enabled ? 'ghost' : '') + ' toggle-job" data-name="' + escapeHtml(j.name) + '" data-enabled="' + (enabled ? '1' : '0') + '" title="' + (enabled ? t('pause') : t('resume')) + '">' + (enabled ? t('pause') : t('resume')) + '</button>';
    html += '<button class="mini-btn edit-job" data-name="' + escapeHtml(j.name) + '">' + t('edit') + '</button><button class="mini-btn ghost del-job" data-name="' + escapeHtml(j.name) + '">' + t('del') + '</button></div></div>';
  }
  html += '</div>';
  body.innerHTML = html;
  ...
  $('addJobBtn').onclick = function() {
    ...
    wireJobForm('newJob');
    var save = document.createElement('button'); save.className = 'mini-btn'; save.textContent = t('save');
    save.onclick = async function() {
      var data = wireJobForm('newJob')();
      if (!data.name) { showToast(t('fill_name'), 'warning'); return; }
      ...
      if (r.ok) { showToast(t('created'), 'success'); renderTasks(); }
      else showToast(t('create_fail') + ': ' + (r.error || ''), 'error');
    };
  ...
  body.querySelectorAll('.run-job').forEach(function(b) { b.onclick = async function() {
    ...
    b.disabled = true; b.textContent = t('running') + '…';
    try {
      var r = await fetch('/api/jobs/' + encodeURIComponent(name), { method: 'POST' }).then(function(x) { return x.json(); });
      if (r.ok) showToast(t('triggered') + ': ' + name, 'success');
      else showToast(t('trigger_fail') + ': ' + (r.error || r.content || ''), 'error');
    } catch(e) { showToast(t('trigger_err'), 'warning'); }
    renderTasks();
  }; });
  ...
  body.querySelectorAll('.toggle-job').forEach(function(b) { b.onclick = async function() {
    ...
    var action = enabled ? t('pause') : t('resume');
    ...
    if (r.ok) { showToast(action + ' ' + t('op_ok') + ': ' + name, 'success'); renderTasks(); }
    else showToast(action + ' ' + t('fail') + ': ' + (r.error || ''), 'error');
  }; });
  ...
  body.querySelectorAll('.edit-job').forEach(function(b) { b.onclick = function() {
    ...
    wireJobForm('editJob');
    var save = document.createElement('button'); save.className = 'mini-btn'; save.textContent = t('save_change');
    save.onclick = async function() {
      ...
      if (r.ok) { showToast(t('saved'), 'success'); renderTasks(); }
      else showToast(t('save_fail'), 'error');
    };
    wrap.appendChild(save);
  }; });
  body.querySelectorAll('.del-job').forEach(function(b) { b.onclick = async function() {
    if (!confirm(t('del_job_confirm', { n: b.dataset.name }))) return;
    var r = await fetch('/api/jobs/' + encodeURIComponent(b.dataset.name), { method: 'DELETE' }).then(function(x) { return x.json(); });
    if (r.ok) showToast(t('deleted'), 'success'); else showToast(t('del_fail'), 'error');
    renderTasks();
  }; });
}
...
function showEditModelDialog(id) {
  ...
  var typeMap = { ollama: 'Ollama', local: t('type_local'), cloud: t('type_cloud') };
  overlay.innerHTML = '\
    <div class="modal-card">\
      <div class="modal-header">' + t('edit_model') + '</div>\
      <div class="field"><label>' + t('type') + '</label><select id="editType">' +
        Object.keys(typeMap).map(function(k) { return '<option value="' + k + '"' + (m.type === k ? ' selected' : '') + '>' + typeMap[k] + '</option>'; }).join('') +
      '</select></div>\
      <div class="field"><label>' + t('display_name') + '</label><input id="editLabel" value="' + escapeHtml(m.label || '') + '" /></div>\
      <div class="field"><label>' + t('model_name') + '</label><input id="editModel" value="' + escapeHtml(m.model || '') + '" /></div>\
      <div class="field"><label>Base URL</label><input id="editBase" value="' + escapeHtml(m.base || '') + '" placeholder="' + t('model_optional') + '" /></div>\
      <div class="field"><label>API Key</label><input id="editKey" type="password" value="' + escapeHtml(m.key || '') + '" placeholder="' + t('model_optional') + '" /></div>\
      <div class="modal-btns">\
        <button class="mini-btn" id="editCancel">' + t('cancel') + '</button>\
        <button class="mini-btn" id="editSave" style="background:var(--accent);color:#fff">' + t('save') + '</button>\
      </div>\
    </div>';
  ...
  overlay.querySelector('#editSave').onclick = async function() {
    ...
    if (!model) { showToast(t('fill_model'), 'warning'); return; }
    ...
    if (!r.ok) { showToast(t('save_fail'), 'error'); return; }
    showToast(t('updated'), 'success'); closeDialog(); renderModels();
  };
...
function showAddModelDialog() {
  ...
  overlay.innerHTML = '\
    <div class="modal-card">\
      <div class="modal-header">' + t('add_model') + '</div>\
      <div class="field"><label>' + t('type') + '</label><select id="addType">' +
        Object.keys(typeMap).map(function(k) { return '<option value="' + k + '">' + typeMap[k] + '</option>'; }).join('') +
      '</select></div>\
      <div class="field"><label>' + t('display_name') + '</label><input id="addLabel" placeholder="' + t('model_name_ph') + '" /></div>\
      <div class="field"><label>' + t('model_name') + '</label><input id="addModel" placeholder="如 qwen3.5:9b / gpt-4o" /></div>\
      <div class="field"><label>Base URL</label><input id="addBase" placeholder="' + t('model_optional') + '" /></div>\
      <div class="field"><label>API Key</label><input id="addKey" type="password" placeholder="' + t('model_key_hint') + '" /></div>\
      <div class="modal-btns">\
        <button class="mini-btn" id="addCancel">' + t('cancel') + '</button>\
        <button class="mini-btn" id="addSave" style="background:var(--accent);color:#fff">' + t('save') + '</button>\
      </div>\
    </div>';
  document.body.appendChild(overlay);
  function closeDialog() { overlay.remove(); }
  ...
  overlay.querySelector('#addSave').onclick = async function() {
    ...
    if (!r.ok) { showToast(t('add_fail'), 'error'); return; }
    showToast(t('model_added'), 'success'); closeDialog(); renderModels();
  };
}
...
async function renderModels() {
  var body = $('panelBody');
  body.innerHTML = '<div class="card"><div class="card-h">' + t('models_avail') + '</div><div class="model-loading"><div class="spinner"></div><div class="ml-text">' + t('models_loading') + '</div></div></div>';
  ...
  try {
    ...
    clearTimeout(to);
    body.innerHTML = '<div class="card"><div class="card-h">' + t('models_slow') + '</div><div class="model-loading"><div class="ml-text">' + t('models_wait') + '</div><button class="mini-btn primary" id="retryModels">' + t('retry') + '</button></div></div>';
    var rb = $('retryModels'); if (rb) rb.onclick = renderModels;
    return;
  }
  clearTimeout(to);
  var html = '<div class="card"><div class="card-h">' + t('models_avail') + '</div>';
  for (var i = 0; i < (md.groups || []).length; i++) {
    ...
    for (var j = 0; j < g.providers.length; j++) {
      ...
      for (var k = 0; k < p.models.length; k++) {
        ...
        html += '<div class="list-row model-row' + (sel ? ' selected' : '') + '" data-id="' + escapeHtml(m.id) + '"><div class="lr-main"><div class="lr-title">' + escapeHtml(m.label) + '</div></div>' + (sel ? '<span class="badge">' + t('default_badge') + '</span>' : '') + '</div>';
      }
    }
  }
  html += '</div>';
  var customs = cfg.customModels || []; _customModels = customs;
  html += '<div class="card"><div class="card-h">' + t('models_custom', { n: customs.length }) + '</div>';
  for (var i = 0; i < customs.length; i++) {
    var c = customs[i];
    html += '<div class="list-row"><div class="lr-main"><div class="lr-title">' + escapeHtml(c.label || c.model) + '</div><div class="lr-sub">' + (c.type || '') + ' · ' + escapeHtml(c.model || '') + '</div></div><span class="lr-acts"><button class="mini-btn warn edit-custom" data-id="' + escapeHtml(c.id) + '">' + t('edit') + '</button><button class="mini-btn danger rm-custom" data-id="' + escapeHtml(c.id) + '">' + t('del_msg') + '</button></span></div>';
  }
  html += '<button class="mini-btn primary" id="addCustomLink">' + t('add_custom') + '</button></div>';
  body.innerHTML = html;
  body.querySelectorAll('.model-row').forEach(function(r) { r.onclick = async function() {
    ...
    var lbl = labelOf(id); if (lbl) setConn(t('connected') + ' · ' + lbl, 'ok');
    showToast(t('model_set'), 'success'); renderModels();
  }; });
  body.querySelectorAll('.rm-custom').forEach(function(b) { b.onclick = async function() {
    ...
    if (!r.ok) { showToast(t('del_fail'), 'error'); return; }
    showToast(t('deleted'), 'success'); renderModels();
  }; });
...
async function renderSkills() {
  ...
  html += '<div class="card"><div class="card-h">' + t('skills_add') + '</div>' +
    '<div class="field"><label>' + t('sk_name') + '</label><input id="skName" placeholder="例如：PDF 摘要"/></div>' +
    '<div class="field"><label>' + t('sk_desc') + '</label><input id="skDesc" placeholder="这个技能做什么"/></div>' +
    '<div class="field"><label>' + t('sk_trigger_ph') + '</label><input id="skTrigger" placeholder="例如：pdf 摘要"/></div>' +
    '<div class="field"><label>' + t('sk_body_ph') + '</label><textarea id="skBody" placeholder="分步骤描述技能的执行方式…"></textarea></div>' +
    '<button class="mini-btn" id="addSkillBtn">' + t('create_skill') + '</button>' +
    '<div class="muted" style="margin-top:8px">' + t('skill_hint') + '</div></div>';
  body.innerHTML = html;
  $('addSkillBtn').onclick = async function() {
    var name = $('skName').value.trim();
    if (!name) { showToast(t('fill_skill'), 'warning'); return; }
    ...
    if (r.ok) { showToast(t('skill_added'), 'success'); renderSkills(); }
    else showToast(t('add_fail') + ': ' + (r.error || ''), 'error');
  };
}
...
async function renderPlugins() {
  ...
  var body = $('panelBody');
  var html = '<div class="card"><div class="card-h">' + t('tools_reg', { n: (d.tools || []).length }) + '</div>';
  for (var i = 0; i < (d.tools || []).length; i++) {
    var t = d.tools[i];
    html += '<div class="list-row"><div class="lr-main"><div class="lr-title">' + escapeHtml(t.name) + (t.custom ? ' <span class="tag">' + t('custom') + '</span>' : '') + '</div><div class="lr-sub">' + escapeHtml(t.description || '') + '</div></div></div>';
  }
  html += '</div>';
  if ((d.custom || []).length) {
    html += '<div class="card"><div class="card-h">' + t('custom_plugins', { n: d.custom.length }) + '</div>';
    for (var i = 0; i < d.custom.length; i++) {
      var c = d.custom[i];
      html += '<div class="list-row"><div class="lr-main"><div class="lr-title">' + escapeHtml(c.name) + '</div><div class="lr-sub">' + escapeHtml(c.command || '') + '</div></div><button class="mini-btn rm-plugin" data-id="' + escapeHtml(c.id) + '">' + t('del') + '</button></div>';
    }
    html += '</div>';
  }
  html += '<div class="card"><div class="card-h">' + t('add_plugin') + '</div><div class="muted">' + t('plugin_desc') + '</div>' +
    '<div class="field"><label>' + t('pl_name') + '</label><input id="plName" placeholder="如: weather"/></div>' +
    '<div class="field"><label>' + t('pl_desc') + '</label><input id="plDesc" placeholder="查询天气"/></div>' +
    '<div class="field"><label>' + t('pl_cmd') + '</label><input id="plCmd" placeholder="如: node tools/weather.js"/></div>' +
    '<button class="mini-btn" id="addPluginBtn">' + t('add_plugin_btn') + '</button></div>';
  body.innerHTML = html;
  ...
  $('addPluginBtn').onclick = async function() {
    ...
    if (r.ok) { showToast(t('plugin_added'), 'success'); renderPlugins(); }
    else showToast(t('add_fail') + ': ' + (r.error || ''), 'error');
  };
}
...
async function renderConfig() {
  ...
  var body = $('panelBody');
  body.innerHTML = '<div class="card"><div class="card-h">' + t('cfg_title') + '</div><pre class="code">' + escapeHtml(JSON.stringify(c, null, 2)) + '</pre><button class="mini-btn" id="openCfgBtn">' + t('open_cfg') + '</button></div>';
...
async function renderLogs() {
  ...
  var body = $('panelBody');
  var html = '<div class="card"><div class="card-h">' + t('logs_title', { n: d.count }) + '</div><div class="logs">';
  ...
  html += '</div><button class="mini-btn" id="refreshLogs">' + t('refresh') + '</button></div>';
  body.innerHTML = html;
...
async function renderMetrics() {
  ...
  html += stat(t('m_uptime'), fmtUptime(m.uptime));
  html += stat(t('m_node'), m.node);
  html += stat(t('m_platform'), m.platform);
  html += stat(t('m_ws'), m.wsConnections);
  html += stat(t('m_rss'), mb(m.memory.rss));
  html += stat(t('m_heap'), mb(m.memory.heapUsed) + ' / ' + mb(m.memory.heapTotal));
  html += stat(t('m_provider'), m.config.provider);
  html += stat(t('m_model'), m.config.activeModelId || '—');
  html += '</div><button class="mini-btn" id="refreshMetrics">' + t('refresh') + '</button>';
  body.innerHTML = html;
...
async function renderHealth() {
  ...
  var body = $('panelBody');
  var html = '<div class="card"><div class="card-h">' + t('health_title')
    + ' <span class="badge" style="background:var(--ok)">' + t('ok_badge') + ' ' + okCount + '</span>'
    + ' <span class="badge" style="background:var(--danger)">' + t('alert_badge') + ' ' + alertCount + '</span>'
    + ' <span style="color:var(--muted);font-weight:400">' + t('health_sub', { n: d.total || 0 }) + '</span></div>';
  if (!alerts.length) {
    html += '<div class="empty-panel">' + t('health_none') + '<br><span style="color:var(--muted)">' + t('health_none_sub') + '</span></div>';
  } else {
    for (var j = 0; j < alerts.length; j++) {
      var a = alerts[j];
      var badge = a.ok
        ? '<span class="badge" style="background:var(--ok)">' + t('ok_badge') + '</span>'
        : '<span class="badge" style="background:var(--danger)">' + t('alert_badge') + '</span>';
  ...
  html += '</div><button class="mini-btn ghost" id="refreshHealth">' + t('refresh') + '</button>';
  body.innerHTML = html;
  $('refreshHealth').onclick = renderHealth;
}

/* ─── 面板: Dashboard 总览（聚合 指标 / 健康 / 日志） ─── */
async function renderDashboard() {
  var body = $('panelBody');
  if (body) body.innerHTML = '<div class="empty-panel">' + t('loading') + '</div>';
  var results = await Promise.all([
    fetch('/api/metrics').then(function(r){return r.json();}).catch(function(){return {uptime:0,node:'?',platform:'?',wsConnections:0,memory:{rss:0,heapUsed:0,heapTotal:0},config:{provider:'?',activeModelId:'-'}};}),
    fetch('/api/health/alerts?limit=300').then(function(r){return r.json();}).catch(function(){return {count:0,total:0,alerts:[]};}),
    fetch('/api/logs').then(function(r){return r.json();}).catch(function(){return {count:0,logs:[]};})
  ]);
  var m = results[0], h = results[1], lg = results[2];
  function mb(b){return (b/1048576).toFixed(1)+' MB';}
  function stat(k,v){return '<div class="stat"><div class="stat-k">'+escapeHtml(k)+'</div><div class="stat-v">'+escapeHtml(String(v))+'</div></div>';}
  var alerts = h.alerts||[];
  var okCount=0, alertCount=0;
  for (var i=0;i<alerts.length;i++){ if(alerts[i].ok) okCount++; else alertCount++; }
  var html = '';
  html += '<div class="card"><div class="card-h">'+t('p_metrics')+'</div><div class="grid2">';
  html += stat(t('m_uptime'), fmtUptime(m.uptime));
  html += stat(t('m_node'), m.node);
  html += stat(t('m_platform'), m.platform);
  html += stat(t('m_ws'), m.wsConnections);
  ...
  html += stat(t('m_model'), m.config.activeModelId||'—');
  html += '</div></div>';
  html += '<div class="card"><div class="card-h">'+t('health_title')
    +' <span class="badge" style="background:var(--ok)">'+t('ok_badge')+' '+okCount+'</span>'
    +' <span class="badge" style="background:var(--danger)">'+t('alert_badge')+' '+alertCount+'</span>'
    +' <span style="color:var(--muted);font-weight:400">'+t('health_sub',{n:h.total||0})+'</span></div>';
  if(!alerts.length){ html += '<div class="empty-panel">'+t('health_none')+'<br><span style="color:var(--muted)">'+t('health_none_sub')+'</span></div>'; }
  else {
    for (var j=0;j<alerts.length;j++){ var a=alerts[j];
      var badge = a.ok ? '<span class="badge" style="background:var(--ok)">'+t('ok_badge')+'</span>' : '<span class="badge" style="background:var(--danger)">'+t('alert_badge')+'</span>';
      html += '<div class="list-row"><div class="lr-main"><div class="lr-title">'+badge+' '+escapeHtml(a.check||'check')+'</div><div class="lr-sub">'+escapeHtml(a.ts||'')+'</div></div><div class="lr-sub" style="text-align:right;max-width:55%;overflow-wrap:anywhere">'+escapeHtml(a.detail||'')+'</div></div>';
    }
  }
  html += '</div>';
  var logs = (lg.logs||[]).slice().reverse();
  html += '<div class="card"><div class="card-h">'+t('logs_title',{n:lg.count||0})+'</div><div class="logs">';
  for (var k=0;k<logs.length;k++){ var l=logs[k]; html += '<div class="log-line log-'+escapeHtml(l.level||'info')+'"><span class="log-t">'+fmtTime(l.t)+'</span>'+escapeHtml(l.msg)+'</div>'; }
  html += '</div></div>';
...
/* ─── 语言切换：重渲染当前视图 ─── */
function updateHint() {
  var el = $('hintText'); if (!el) return;
  var lbl = labelOf(currentModelId) || '—';
  el.innerHTML = t('hint_pre') + '<b id="footModel">' + escapeHtml(lbl) + '</b>' + t('hint_post');
}
$('langBtn').onclick = function() { toggleLang(); };
window.applyLangChange = function() {
  buildNav();
  updateModelTrigger();
  updateHint();
  updateCtxBadge(0);
  updateInputTokens();
  updateMsgCount();
  var es = $('emptyState'); if (es) es.innerHTML = emptyStateHtml();
  if (currentView && currentView !== 'chat' && currentView !== 'settings') {
    var map = { history: renderHistory, memory: renderMemory, tasks: renderTasks, models: renderModels, skills: renderSkills, plugins: renderPlugins, config: renderConfig, logs: renderLogs, metrics: renderMetrics, health: renderHealth, dashboard: renderDashboard };
...
var UI_DEFAULTS = {
  ...
  voiceInput: true,
  voiceTTS: true,
  labelUser: t('label_you'),
  labelAgent: t('label_assist'),
...
var AV = {
  user: [
    { id:'wheel', label: t('avatar_wheel'), c1:'#14b8a6', c2:'#0f766e', ring:'rgba(20,184,166,.42)',
    ...
    { id:'anchor', label: t('avatar_anchor'), c1:'#38bdf8', c2:'#0369a1', ring:'rgba(56,189,248,.42)',
    ...
    { id:'lighthouse', label: t('avatar_lighthouse'), c1:'#fb923c', c2:'#c2410c', ring:'rgba(251,146,60,.42)',
    ...
    { id:'compass', label: t('avatar_compass'), c1:'#34d399', c2:'#047857', ring:'rgba(52,211,153,.42)',
    ...
    { id:'wave', label: t('avatar_wave'), c1:'#22d3ee', c2:'#0e7490', ring:'rgba(34,211,238,.42)',
    ...
    { id:'ship', label: t('avatar_ship'), c1:'#f59e0b', c2:'#b45309', ring:'rgba(245,158,11,.42)',
  ...
  agent: [
    { id:'core', label: t('avatar_core'), c1:'#8b5cf6', c2:'#6d28d9', ring:'rgba(139,92,246,.45)',
    ...
    { id:'bot', label: t('avatar_bot'), c1:'#60a5fa', c2:'#2563eb', ring:'rgba(96,165,250,.45)',
    ...
    { id:'chip', label: t('avatar_chip'), c1:'#2dd4bf', c2:'#0d9488', ring:'rgba(45,212,191,.45)',
    ...
    { id:'spark', label: t('avatar_spark'), c1:'#f472b6', c2:'#be185d', ring:'rgba(244,114,182,.45)',
    ...
    { id:'rocket', label: t('avatar_rocket'), c1:'#fb7185', c2:'#e11d48', ring:'rgba(251,113,133,.45)',
    ...
    { id:'orbit', label: t('avatar_orbit'), c1:'#a78bfa', c2:'#7c3aed', ring:'rgba(167,139,250,.45)',
...
function toggleSpeak(el, btn, text) {
  if (!('speechSynthesis' in window)) { showToast(t('tts_unsupported'), 'warning'); return; }
  if (_speakingEl === el) { window.speechSynthesis.cancel(); _speakingEl = null; btn.textContent = t('speak'); btn.classList.remove('on'); return; }
  window.speechSynthesis.cancel();
  var u = new SpeechSynthesisUtterance(String(text || '').slice(0, 600));
  u.lang = 'zh-CN'; u.rate = 1;
  u.onend = u.onerror = function() { _speakingEl = null; btn.textContent = t('speak'); btn.classList.remove('on'); };
  window.speechSynthesis.speak(u);
  _speakingEl = el; btn.textContent = t('stop'); btn.classList.add('on');
}
...
function bindChatSettings() {
  ...
  var lu = $('setLabelUser'); if (lu) lu.onchange = function(e) { chatUI.labelUser = e.target.value.trim() || t('label_you'); saveUI(); applyMsgLabels(); };
  var la = $('setLabelAgent'); if (la) la.onchange = function(e) { chatUI.labelAgent = e.target.value.trim() || t('label_assist'); saveUI(); applyMsgLabels(); };
  var cc = $('clearChatBtn'); if (cc) cc.onclick = function() { if (confirm(t('clear_confirm'))) { newChat(); closeChatSettings(); } };
...
function applyMsgLabels() {
  document.querySelectorAll('.msg').forEach(function(m) {
    ...
    if (m.classList.contains('user')) who.textContent = chatUI.labelUser || t('label_you');
    else if (m.classList.contains('assistant')) who.textContent = chatUI.labelAgent || t('label_assist');
  });
}
